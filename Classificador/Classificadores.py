...
for axi, i in zip(ax, range(0,6,3)):
    train=axi.scatter(X_train[:,i], X_train[:,i+2],c=Y_train, s=50,cmap='bwr')
    #test=axi.scatter(X_test[:,i], X_test[:,i+2],c='purple', s=10,cmap='autumn', label='Validação')
    axi.set_title(title[i], size=14)
    axi.set_xlabel('C3')
    axi.set_ylabel('C4')
    axi.legend(*train.legend_elements(),title='Treinamento')
